...
result <- experiment(n1, n2, p1, p2, sigma1, sigma2, 500, cl)

PV_M <- result[[1]]
PW_M <- result[[2]]
```

#### График зависимости достигаемого уровня значимости критерия Фишера от значений $\sigma_{2}$ и $p1=p2=p$, усреднённого по большому количеству повторений эксперимента.

```{r}
drawExp(result[[1]], sigma2, p1, "Fisher Test p-values")
```

#### График с эмпирической оценкой мощности критерия Фишера.

```{r}
drawExp(result[[2]], sigma2, p1, "Fisher Test power")
```